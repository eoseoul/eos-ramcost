 ...
 */
#include "eosio.system.ramcost.hpp"
...
namespace eosiosystem {
   ...
   void system_contract::biosramcost( account_name boot, asset quant ) {
      require_auth(boot);
      eosio_assert( quant.amount > 0, "must update a positive amount of bytes of quote" );
      auto itr = _rammarket.find(S(4,RAMCORE));
      _rammarket.modify( itr, 0, [&]( auto& es ) {
          es.bios_account_cost_mitigate_quote(quant);
      });
   }
